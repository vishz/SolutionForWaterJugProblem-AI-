...
//Check if the current node is an already visited node
bool checkState(queue<nodes> allStates, nodes state){
	queue<nodes> tempQ = allStates;
	
	while(!tempQ.empty()){
		if(tempQ.front().pairStates==state.pairStates) 
			return 0;
		else{
			tempQ.pop();
		}
...
int main()
{
    ...
    queue<nodes> basicQueue, allStates;
    ...
    while(!basicQueue.empty()){    	
    	...
    	if(jug.pairStates.first == final.pairStates.first && jug.pairStates.second == final.pairStates.second){
	    	cout<<jug.stringPairStates;
	    	cout<<"\n";
	    }
	    
	    //Check if the state has already been visited
    	if(checkState(allStates,jug)){
	    	allStates.push(jug);	    	
	    	nodes temp = jug;
